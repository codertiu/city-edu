            ...
            <div class="col-lg-4 form-horizontal">
                <div class="form-group form-material">
                    <div class=" col-lg-12 col-sm-9">
                        <?
                        if (Yii::$app->session->get('type_of_reg') == 2) {
                            $model->call_name = 1;
                            ?>
                            <?= $form->field($model, 'call_name', [
                                'template' => '{label} * {input}{error}{hint}'
                            ])->radioList([
                                1 => Yii::$app->params['call_name'][1]
                            ], [
                                    'item' => function ($index, $label, $name, $checked, $value) {
                                        $check = ($value == $checked) ? 'checked' : '';
                                        $return = '<div class="radio-custom radio-primary radio-inline">';
                        ...
                        <? } else { ?>
                            <?= $form->field($model, 'call_name', [
                                ...
                                2 => Yii::$app->params['call_name'][2],
                                3 => Yii::$app->params['call_name'][3]
                            ], [
                                    'item' => function ($index, $label, $name, $checked, $value) {
                                        $check = ($value == $checked) ? 'checked' : '';
                                        $return = '<div class="radio-custom radio-primary radio-inline">';
...
<?php
$js = <<<JS
    // $('#reception-tel').keyup(function(){
    //    $('#check-number').hide();   
    // });
    
    $('#reception-tel').keypress(function(){
        $.ajax({
            url:'number',
            data:{data:$('#reception-tel').val()},
            type:'POST',
            success:function(res){
                $('#check-number').html(res).show();
            },
            error:function(res){
                console.log(res);
            }
